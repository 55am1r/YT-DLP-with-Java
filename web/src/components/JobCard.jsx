  ...
  return (
    <div className={`job glass ${bad ? 'job-failed' : ''} ${done ? 'job-done' : ''}`}>
      ...
      <div className="job-info">
        {job.status === 'DOWNLOADING' && job.speed && <span>⬇ <b>{job.speed}</b></span>}
        {job.status === 'DOWNLOADING' && job.eta && <span>ETA <b>{job.eta}</b></span>}
      ...
      <div className="job-foot">
        ...
        <div className="job-actions">
          {job.status === 'DOWNLOADING' && (
            <button className="btn btn-sm" onClick={() => pauseJob(job.id)}>⏸ Pause</button>
          )}
          {paused && <button className="btn btn-sm" onClick={() => resumeJob(job.id)}>▶ Resume</button>}
          {ACTIVE.has(job.status) && (
            <button className="btn btn-sm btn-ghost danger" onClick={() => cancelJob(job.id)}>✕ Cancel</button>
          )}
          {done && (
            <a className="btn btn-primary btn-sm" href={blobUrl || fileUrl(job.id)} download={job.fileName || true}>
              ⬇ Save file{blobUrl ? ' (ready)' : ''}
            </a>
          )}
        </div>
      </div>
